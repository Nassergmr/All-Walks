...
import { getJordanBrand } from "@/services/productServices";
...
const Section3: React.FC = () => {
  ...
  const jordanBrandSliced = jordanBrand.slice(0, -2);

  const { ref, inView } = useInView();

  useEffect(() => {
    const fetchedProducts = async () => {
      const data = await getJordanBrand();
      setJordanBrand(data.data);
    };
  ...
  return (
    <div className=" sm:my-[4rem] my-[2rem] sm:px-[20px] px-0 grid md:grid-cols-2 grid-cols-1 gap-2 gap-y-5 justify-between">
      <div id="left">
        <div
          id="product_card"
          className="relative mx-auto h-[80vh] md:h-[700px] flex overflow-hidden group"
        >
          <div id="img_container" className="md:h-[700px]">
            <Image
              src="/images/home-images/pexels-mark-joey-balani-434394388-15298946-Photoroom (1).png"
              fill
              alt=""
